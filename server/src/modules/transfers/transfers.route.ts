...
function requireStatus(transfer: { status: string }, allowed: string[]) {
  if (!allowed.includes(transfer.status)) throw new HttpError(400, 'InvalidTransferStatus');
}
...
transfersRouter.post('/transfers/:id/approve', requireAuth, requireRoleAtLeast(Role.MANAGER), async (req, res, next) => {
  try {
    ...
    if (!transfer) throw new HttpError(404, 'TransferNotFound');
