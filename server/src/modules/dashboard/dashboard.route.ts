...
dashboardRouter.get('/dashboard/stats', requireAuth, async (req, res, next) => {
  try {
    ...
    const [kitchensCount, lowStockItems, depletionStats, recentLedger] = await Promise.all([
      ...
      kitchenId
        ? prisma.kitchenStock
            ...
            .then((rows: Array<{ item: { reorderPoint: number }; onHandQty: number }>) =>
              rows.filter((r) => r.item.reorderPoint > 0 && r.onHandQty <= r.item.reorderPoint),
            )
        : [],
      kitchenId
    ...
    const depletionByType = depletionStats.reduce(
      (acc: Record<string, { totalQty: number; count: number }>, s: { type: string; _sum: { qtyDelta: number | null }; _count: number }) => {
    ...
    const totalDepletion7d =
      (depletionByType.TRANSFER_OUT?.totalQty ?? 0) +
    ...
    res.json({
      ...
      stats: {
        ...
        lowStockItems: lowStockItems.map((s: { itemId: string; item: { name: string; uom: string; reorderPoint: number }; onHandQty: number }) => ({
          itemId: s.itemId,
        ...
        depletionByType,
        recentLedger: recentLedger.map((l: { id: string; type: string; qtyDelta: number; item: { name: string; uom: string }; createdAt: Date }) => ({
          id: l.id,
          type: l.type,
          qtyDelta: l.qtyDelta,
